...
import Available from '../features/masterAvailable.js';
import Awaiting from '../features/masterAwaiting.js';
import History from '../features/masterHistory.js';
...
class Master extends Component {
  ...
  render() {
    ...
    this.props.events && this.props.events.map(event => {
      if (this.props.users[event.authorEmail].organization === this.props.users[this.props.auth.email].organization) {
        history.push(event)

        if (event.action === 'Approved' && event.result === 'Approved') {
          return awaiting.push(event)
        }

        else if (event.action === 'Pending' && event.result === 'Pending') {
          return available.push(event)
        }
    ...
    return (
      <div>
        ...
        {(this.state.messageWindow) ?
          <Grid container direction = 'row'>
            <Grid item style = {{width: table_width, maxWidth: table_width, maxHeight: window.innerHeight, overflow: 'auto'}}>
              <Available rows = {available} margin = {false} />
              <Awaiting rows = {awaiting} margin = {false} />
        ...
        :
          <div>
            <Available rows = {available} margin = {true} />
            <Awaiting rows = {awaiting} margin = {true} />
...
export default compose(connect(mapStateToProps, mapDispatchToProps), firestoreConnect([{collection: 'users'}, {collection: 'events', orderBy: ['dateSubmitted', 'desc']}, {collection: 'messages', orderBy: ['timeSubmitted', 'desc']}]))(Master)
