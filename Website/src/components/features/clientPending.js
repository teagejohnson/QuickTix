...
function Pending({rows, margin}) {
  ...
  return (
    <Box ml = {5} mr = {mr()} mt = {5} mb = {5}>
      <Typography style = {{marginTop: 50, marginBottom: 10}} variant = 'h6' align = 'center'> Pending Approval </Typography>
        <Table size = 'small'>
          <TableHead>
            <TableRow>
                  ...
                  >
                    <Box ml = {3} mr = {3} mt = {1} mb = {1}>
                      <Grid container direction = 'column' alignItems = 'center' spacing = {1}>
                        ...
                        <Grid item>
                          <Button onClick = {updateResult(row.id, 'Canceled', 'Not Available')}> Not Available </Button>
                        </Grid>
                        <Grid item>
                          <Button onClick = {updateResult(row.id, 'Canceled', 'Info Changed')}> Info Changed </Button>
                        </Grid>
                        <Grid item>
                          <Button onClick = {updateResult(row.id, 'Canceled', 'Exceeded Limit')}> Exceeded Limit </Button>
                        </Grid>
                        <Grid item>
      ...
      {(rows.length === 0) ?
        <Box mt = {2}>
          <Typography align = 'center'> No Tickets Pending Approval </Typography>
        </Box>
      : null}
    </Box>
  )
